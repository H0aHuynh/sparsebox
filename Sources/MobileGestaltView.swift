...
private let islandName = "iPhone Air"
	...
	func performApplyMobileGestalt() async throws {
		...
		if bookassetdUUID == nil {
			...
			print("Tìm kiếm container bookassetd UUID...")
		...
		print("Đang vá BLDatabaseManager.sqlite...")
		...
		if let pid_bookassetd {
			print("Đang dừng bookassetd (pid \(pid_bookassetd))...")
			try context?.killProcess(withPID: pid_bookassetd, signal: SIGSTOP)
		}
		if let pid_Books {
			print("Đang tắt Books (pid \(pid_Books))...")
		...
		if let pid_itunesstored {
			print("Đang tắt itunesstored (pid \(pid_itunesstored))...")
		...
		if let pid_bookassetd {
			print("Đang tắt bookassetd (pid \(pid_bookassetd))...")
